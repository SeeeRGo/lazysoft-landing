...
test.each([false,true])('anonymous=%s: resumable submission survives a lost HTTP response and never waits for inline Telegram',{timeout:15000},async(anonymous)=>{
 ...
 const send=()=>fetch(`http://127.0.0.1:${port}/api/mvp-request`,{method:'POST',headers:{'content-type':'application/json',origin:`http://127.0.0.1:${port}`},body:JSON.stringify({requestType:'mvp',idea:'Сайт магазина инструментов с каталогом',...(anonymous?{contactMethod:'none',contact:'stale@example.org'}:{contactMethod:'telegram',contact:'@test_example'}),submissionToken:'b'.repeat(43)})});
 expect((await send()).status).toBe(500);const response=await send();expect(response.status).toBe(200);const data=await response.json();expect(data).toMatchObject({stored:true,notificationQueued:true,accessToken:'b'.repeat(43),requestId:row.requestId});expect(posts).toBe(2);expect(row.contactMethod).toBe(anonymous?'none':'telegram');expect(row.contact).toBe(anonymous?'':'@test_example');
 const invalid=await fetch(`http://127.0.0.1:${port}/api/mvp-request`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({requestType:'crm',idea:'Нужна доработка CRM по заявкам',submissionToken:'c'.repeat(43)})});expect(invalid.status).toBe(400);expect(posts).toBe(2);
