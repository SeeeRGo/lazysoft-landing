...
test.each([false,true])('anonymous=%s: resumable submission survives a lost HTTP response and never waits for inline Telegram',{timeout:15000},async(anonymous)=>{
 ...
 const send=()=>fetch(`http://127.0.0.1:${port}/api/mvp-request`,{method:'POST',headers:{'content-type':'application/json',origin:`http://127.0.0.1:${port}`},body:JSON.stringify({requestType:'mvp',idea:'Сайт магазина инструментов с каталогом',...(anonymous?{}:{contactMethod:'telegram',contact:'@test_example'}),submissionToken:'b'.repeat(43)})});
