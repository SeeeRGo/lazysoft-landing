  ...
  handler: async (ctx, args): Promise<null> => {
    ...
    try {
      ...
      if (delivery.telegramChatId) {
        ...
        try {
          const result = await transport.sendMail({ from: process.env.SMTP_FROM, to: delivery.contact, subject: `ТЗ и демо вашей идеи · Lazysoft ${delivery.requestId}`, text, messageId: `<${args.deliveryId}@lazysoft.ru>`, disableFileAccess: true, disableUrlAccess: true });
