...
Генерация PDF и ТЗ исключена из новых задач и правок. Модель создаёт только
интерактивное демо и короткую инструкцию запуска; варианты реализации и оценки
автоматически не составляются. Клиент получает ссылку на демо в чате заявки.
Архив содержит `demo/` и `README.md`, без файлов спецификации.
Старые PDF в истории доступны, но для завершения новой задачи PDF не требуется.
Приведённая выше проверка выпуска описывает предыдущую версию с PDF.
