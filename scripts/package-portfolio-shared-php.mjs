// Build the no-SSH shared-hosting PHP/MySQL handover from a trusted SQLite snapshot.
...
if(!process.argv[2]||!existsSync(join(input,'portfolio.sqlite')))throw Error('Usage: node --experimental-sqlite scripts/package-portfolio-shared-php.mjs <snapshot/data>');
...
const source=resolve('standalone/portfolio-shared-php'),dir=mkdtempSync(resolve('.local/portfolio-shared-php-'));chmodSync(dir,0o700);
...
const guides=[
 ['artifacts/instructions/portfolio-hostiman-options-guide.pdf','ИНСТРУКЦИЯ-HOSTIMAN.pdf'],
].filter(([file])=>existsSync(resolve(file)));
for(const [file,name] of guides)cpSync(resolve(file),join(dir,name));
const archive=join(dir,'portfolio-php-mysql.zip'),entries=['public_html','private','INSTALL-KEY.txt','README.md',...guides.map(([,name])=>name)];
