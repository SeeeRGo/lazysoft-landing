...
export function completionMessage(result, kind) {
  ...
  return `${result.title}\n\n${kind === "revision" ? `Версия ${id} готова после доработок. Предыдущие версии сохранены.` : "Первая версия сайта готова по вашей исходной идее."} Откройте результат на странице заявки. В демо нажмите «Панель редактора»: тексты, фото, цены и каталог можно менять самостоятельно, изменения сохраняются в этом браузере. В приобретаемых исходниках — полноценная CMS с сохранением на сервере. Можно купить любую готовую версию или отправить следующее сообщение с доработками, если они ещё доступны. Всего после исходной идеи предусмотрено два сообщения. Данные в демо вымышлены; исходники с админкой и размещение выбираются отдельно.`;
}
