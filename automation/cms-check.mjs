...
export async function checkCms(site,{screenshots,forbidFallback=false}={}){
 ...
 ws.addEventListener('message',e=>{const m=JSON.parse(e.data);if(m.method==='Fetch.requestPaused'){const u=m.params.request.url,allowed=u.startsWith(origin+'/')||u.startsWith('data:image/');void send(allowed?'Fetch.continueRequest':'Fetch.failRequest',{requestId:m.params.requestId,...(allowed?{}:{errorReason:'BlockedByClient'})}).catch(()=>{});if(!allowed)errors.push('External request blocked');}if(m.method==='Runtime.exceptionThrown')errors.push(m.params.exceptionDetails.text);if(m.method==='Network.responseReceived'&&m.params.response.status>=400&&!m.params.response.url.endsWith('favicon.ico'))errors.push('HTTP '+m.params.response.status);if(!m.id)return;const p=pending.get(m.id);pending.delete(m.id);if(p)m.error?p.reject(m.error):p.resolve(m.result)});
 const evaluate=async expression=>{const r=await send('Runtime.evaluate',{expression,returnByValue:true,awaitPromise:true});if(r.exceptionDetails){const detail=r.exceptionDetails.exception?.description||r.exceptionDetails.text||'unknown';throw Error('CMS browser script failed: '+String(detail).split('\n')[0].slice(0,180))}return r.result.value};
 const until=async(expression,label='condition',attempts=100)=>{let lastError='';for(let i=0;i<attempts;i++){try{if(await evaluate(expression))return}catch(error){lastError=String(error?.message||error).replace(/[^A-Za-z0-9 .:_-]/g,'').slice(0,120)}await new Promise(r=>setTimeout(r,100))}throw Error('CMS browser condition timed out: '+label+(lastError?' evaluation='+lastError:''))};
 let navigationId=0;
 const navigate=async path=>{
  const token=String(++navigationId),url=`${origin}/${path}?cms_check=${token}`;
  const result=await send('Page.navigate',{url});
  if(result.errorText)throw Error(`CMS browser navigation failed: ${path}`);
  await until(`location.pathname===${JSON.stringify('/'+path)}&&new URLSearchParams(location.search).get('cms_check')===${JSON.stringify(token)}&&document.readyState==='complete'`,'navigation '+path,150);
 };
 const browserErrors=()=>errors.slice(-4).join(' | ').slice(0,320);
 const navigateAdminCollection=async key=>{for(let attempt=0;attempt<2;attempt++){await navigate('admin.html');try{await until(`!!document.querySelector('[data-add-collection="${key}"]')&&!document.querySelector('#editor').hidden`,'admin collection '+key,150);return}catch(error){if(attempt===1){const detail=await evaluate(`(()=>{const status=document.querySelector('#status')?.textContent?.trim()||'empty';const scripts=Array.from(document.scripts).map(s=>s.getAttribute('src')||'inline').join(',');return ('status='+status+' editorHidden='+document.querySelector('#editor')?.hidden+' addButtons='+document.querySelectorAll('[data-add-collection]').length+' scripts='+scripts).slice(0,320)})()`);throw Error(error.message+`: ${detail}${browserErrors()?` browser=${browserErrors()}`:''}`)}}}};
 ...
 for (let round = 0; round < 10; round += 1) {
  ...
  try{await until(`(()=>{const images=Array.from(document.images).filter(i=>/\\.(?:jpe?g|png|webp)(?:$|[?#])/i.test(i.src));return images.length>=3&&new Set(images.map(i=>new URL(i.src,location.href).pathname)).size>=3&&images.every(i=>i.complete&&i.naturalWidth>0)})()`,'initial raster images')}catch(error){const detail=await evaluate(`(()=>{const images=Array.from(document.images).filter(i=>/\\.(?:jpe?g|png|webp)(?:$|[?#])/i.test(i.src));return ('count='+images.length+' distinct='+new Set(images.map(i=>new URL(i.src,location.href).pathname)).size+' '+images.map(i=>(i.getAttribute('src')||'empty')+':complete='+i.complete+':width='+i.naturalWidth).join(',')).slice(0,500)})()`);throw Error(error.message+`: ${detail}${browserErrors()?` browser=${browserErrors()}`:''}`)};
 ...
 const inertInteractiveCards=await evaluate(`Array.from(document.querySelectorAll('article,li')).filter(card=>/(^|\\s)(?:puzzle|quiz|accordion|selectable)(?:-|\\s|$)/i.test(String(card.className))&&!card.matches('a,button,details,[role="button"]')&&!card.querySelector('a,button,input,select,textarea,details,[role="button"]')).map(card=>String(card.className))`);
 assert.deepEqual(inertInteractiveCards,[],'Interactive-looking cards must contain a real keyboard control');
 const gridChecks=await evaluate(`(async()=>{const out=[];for(const grid of document.querySelectorAll('[role="grid"]')){const cells=Array.from(grid.querySelectorAll('button'));if(cells.length!==64)continue;const snap=()=>{const box=grid.getBoundingClientRect(),rects=cells.map(cell=>cell.getBoundingClientRect()),widths=rects.map(r=>r.width),heights=rects.map(r=>r.height);return{x:box.x,y:box.y,width:box.width,height:box.height,uniform:Math.max(...widths)-Math.min(...widths)<1&&Math.max(...heights)-Math.min(...heights)<1&&Math.abs(widths[0]-heights[0])<1}};const before=snap();cells[48].click();await new Promise(r=>requestAnimationFrame(()=>r()));cells[40].click();await new Promise(r=>setTimeout(r,700));const after=snap();out.push({before,after})}return out})()`);
 for(const check of gridChecks){assert(check.before.uniform&&check.after.uniform,'8x8 grid cells must remain equal before and after interaction');assert(Math.max(...['width','height'].map(key=>Math.abs(check.before[key]-check.after[key])))<1,'8x8 grid size must not change after interaction')}
