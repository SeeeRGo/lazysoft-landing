...
export async function checkCms(site,{screenshots,forbidFallback=false}={}){
 ...
 for (let round = 0; round < 10; round += 1) {
  ...
  try{await until(`(()=>{const images=Array.from(document.images).filter(i=>/\\.(?:jpe?g|png|webp)(?:$|[?#])/i.test(i.src));return images.length>=3&&images.every(i=>i.complete&&i.naturalWidth>0)})()`,'initial raster images')}catch(error){const detail=await evaluate(`(()=>{const images=Array.from(document.images).filter(i=>/\\.(?:jpe?g|png|webp)(?:$|[?#])/i.test(i.src));return ('count='+images.length+' '+images.map(i=>(i.getAttribute('src')||'empty')+':complete='+i.complete+':width='+i.naturalWidth).join(',')).slice(0,500)})()`);throw Error(error.message+`: ${detail}${browserErrors()?` browser=${browserErrors()}`:''}`)};
