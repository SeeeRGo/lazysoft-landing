...
type ContactMethod = "email" | "telegram" | "max";
interface Submission { requestType: "mvp"; idea: string; contact: string; contactMethod: ContactMethod; website: string; source: Record<string, string>; submissionToken: string; startedAt: number; }
const validContactMethod = (value: unknown): value is ContactMethod => value === "email" || value === "telegram" || value === "max";
...
export function setupSiteSubmission(form: HTMLFormElement, track: (name: string, args?: Record<string, unknown>) => void, source: () => Record<string, string>) {
  ...
  if (stored?.requestType === "mvp" && validToken(stored.submissionToken) && typeof stored.idea === "string" && typeof stored.contact === "string" && stored.contact.trim().length >= 3 && validContactMethod(stored.contactMethod) && Number.isFinite(stored.startedAt)) {
    pending = stored;
  } else if (stored?.requestType === "mvp") {
    if (typeof stored.idea === "string") idea.value = stored.idea.slice(0, 3000);
    if (contact && typeof stored.contact === "string") contact.value = stored.contact.slice(0, 200);
    remove(PENDING);
  }
  ...
  async function submit() {
    ...
    if (!pending) {
      ...
      pending = { requestType: "mvp", idea: idea.value.trim(), contact: contact?.value.trim() || "", contactMethod, website: (form.elements.namedItem("website") as HTMLInputElement).value, source: source(), submissionToken, startedAt: Date.now() };
    }
