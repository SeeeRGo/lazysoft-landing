...
const trustedStyle=document.createElement('style');trustedStyle.textContent=':where(a,button,input,textarea,select):focus-visible{outline:3px solid currentColor;outline-offset:3px}:where(.hero,[class*="hero"],[id*="hero"]) img{min-height:80px;aspect-ratio:16/9;object-fit:cover}@media(prefers-reduced-motion:reduce){*,*::before,*::after{scroll-behavior:auto!important;animation-duration:.01ms!important;animation-iteration-count:1!important;transition-duration:.01ms!important}}.lazysoft-admin-link{position:fixed;z-index:2147483000;top:16px;right:16px;display:inline-flex;align-items:center;min-height:44px;padding:10px 16px;border:1px solid currentColor;border-radius:999px;background:#fff;color:#111!important;font:600 14px/1.2 sans-serif;text-decoration:none;box-shadow:0 8px 28px #0002}.lazysoft-cms-fallback-inner{box-sizing:border-box;width:min(100%,1200px);margin:0 auto;padding:clamp(32px,6vw,80px) 24px}.lazysoft-cms-fallback-inner h2{margin:0 0 24px;font:inherit;font-size:clamp(1.5rem,3vw,2.5rem)}.lazysoft-cms-fallback-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(260px,100%),1fr));gap:24px}.lazysoft-cms-fallback-card{min-width:0}.lazysoft-cms-fallback-card img{display:block;width:100%;max-width:100%;aspect-ratio:4/3;object-fit:cover;border-radius:inherit}.lazysoft-cms-fallback-card h3,.lazysoft-cms-fallback-card p{overflow-wrap:anywhere}@media(max-width:500px){.lazysoft-admin-link{top:10px;right:10px;max-width:calc(100vw - 20px)}.lazysoft-cms-fallback-inner{padding:32px 16px}.lazysoft-cms-fallback-grid{gap:16px}}';document.head.append(trustedStyle);
if(!Array.from(document.querySelectorAll('a[href]')).some(anchor=>{try{return new URL(anchor.getAttribute('href'),location.href).pathname.endsWith('/admin.html')}catch{return false}})){const adminLink=document.createElement('a');adminLink.className='lazysoft-admin-link';adminLink.href='admin.html';adminLink.textContent='Редактировать сайт самостоятельно';document.body.prepend(adminLink)}
